...
function move_buildlist($path_to,$path='data',$elem=0,$move_link='')
{
  //////////////////////////////////////////////
  ...
  { 
     ...
     if ($array[$c]==$part[$elem])
     {
       ...
       echo '<li class="dir2"><a href="'.($temp_path ? '?'.$move_link.'&amp;id='.prd($temp_path) : './?'.$move_link.'').'">'.$name.'</a></li>';
       if (func('countfiles',$path.'/'.$array[$c]) > 0 )
         move_buildlist($path_to,$path.'/'.$array[$c],$elem,$move_link);
       else
         echo '<ul><li class="file"><span style="color:rgb(86, 176, 79)">['.$lang['empty'].']</span></li></ul>'; 
     }
     else
       echo '<li class="dir"><a  href="?'.$move_link.'&amp;id='.prd(cut_data($path2.':'.$array[$c])).'">'.$name.'</a></li>';
  }
    ...
    if(mysql_num_rows($req))
    {
      while ($res = mysql_fetch_array($req))
      {
        if(!in_array($res['id'],$id_page_show))
        {
          ...
          if ($mod_rewrite)
            echo '<li class="file"><span style="color:rgb(86, 176, 79)">'.out($res['name']).'</span></li>';
          else
            echo '<li class="file"><span style="color:rgb(86, 176, 79)">'.out($res['name']).'</span></li>';  
